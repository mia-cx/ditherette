...
| [S41](#s41) | Tune complete calls and assemble fresh performance evidence | [S40](#s40), [S20](#s20) | In progress; listed PRs through #200 merged. Integrated scalar qualification is complete, but the across-settings JS target is not. #204 awaits review for nearest-only preparation: Chromium 5/10/25/50% cold improves 6–15%, with 16 exact output pairs. Firefox 25% public-call time stays 3 ms; processing time rises 0.5 ms. Firefox quantization controls rise 2.4% cold and 3.5% warm. Chromium tiny-nearest cold calls, some cold direct recipes, and Firefox 25% fixed Lanczos still miss the target. Guarded byte thresholds retain an empirical, not certified, 1e-9 fallback. Vector-field JS comparisons remain semantic mismatches. #192 row-boundary candidate rejected for warm regressions | [#172](https://github.com/mia-cx/ditherette/pull/172), [#173](https://github.com/mia-cx/ditherette/pull/173), [#175](https://github.com/mia-cx/ditherette/pull/175), [#177](https://github.com/mia-cx/ditherette/pull/177), [#179](https://github.com/mia-cx/ditherette/pull/179), [#181](https://github.com/mia-cx/ditherette/pull/181), [#183](https://github.com/mia-cx/ditherette/pull/183), [#185](https://github.com/mia-cx/ditherette/pull/185), [#188](https://github.com/mia-cx/ditherette/pull/188), [#190](https://github.com/mia-cx/ditherette/pull/190), [#193](https://github.com/mia-cx/ditherette/pull/193), [#195](https://github.com/mia-cx/ditherette/pull/195), [#197](https://github.com/mia-cx/ditherette/pull/197), [#198](https://github.com/mia-cx/ditherette/pull/198), [#200](https://github.com/mia-cx/ditherette/pull/200), [#204](https://github.com/mia-cx/ditherette/pull/204) |
...
The selected #203 preparation change keeps palette and other-filter constructors outside direct sparse-nearest execution. It shares existing cache keys, allocation accounting, publication and nearest kernels. Progress and threaded calls retain general preparation. Explicit developer band overrides still apply.

Six-round scalar pairs at nearest 5/10/25/50% show Chromium cold processing improving from 2.40/3.00/4.95/10.95 to 2.05/2.70/4.65/9.95 ms. Warm changes stay within 0.05 ms or improve. Firefox 25% public-call medians remain 3 ms; processing medians rise from 3 to 3.5 ms, while warm processing falls from 2.5 to 2 ms. Its 1 ms timer leaves small calls inconclusive. Firefox Oklch quantization controls rise 2.4% cold and 3.5% warm. This is a narrow cold-start improvement, not universal JavaScript parity.

All 16 final output pairs are exact, with 864 unchanged stability comparisons. Scalar Wasm grows from 428,094 to 431,027 bytes; accounted Wasm overhead remains 15,820 bytes. The selected digest is `f99727665912b3cfc5430b86ded3bfc039c6425ffa08015cb95089d062d68cd8`. Source patch, package hashes, raw samples and images remain external under `/home/mia/mia-cx/ditherette-performance-20260920`, identified by `nearest-preparation-v2-provenance.json` and `nearest-preparation-v2-qualification-audit.json`. The earlier candidate's initial/reversed manifests retain a copied `typed-key` label; their actual artifact hashes are recorded, and they are not the selected artifact. No 150% timing is claimed because the baseline Firefox renderer failure remains unresolved.

The selected #199 row-reuse candidate keeps the generic gather unchanged and only copies repeated rows in aligned, ordinary-buffer scalar upscales. Shared inputs, threaded offsets, progress calls and unaligned inputs retain existing behavior. Six-round pairs cover nearest 5/10/25/50/150% plus sparse Lanczos2/3 controls in Chromium and optimizing Firefox. A reversed Chromium 25/150% pair confirms the upscale gain; the initial 0.30 ms 25% difference does not repeat. Chromium 5/10% cold calls remain 0.10 ms slower in the fresh pair. Firefox sub-millisecond controls remain timer-floor limited.

At 150%, fresh Chromium public-call medians improve from 62.55/61.95 to 53.65/53.70 ms cold/warm. Firefox improves from 74/76.5 to 61.5/62 ms. End-to-end medians also improve: Chromium 73.45/72.65 to 63.95/62.10 ms, Firefox 83/84.5 to 70.5/71 ms. Reversed Chromium public-call gains are 12% cold and 8% warm. These are narrow paired results, not completion of the across-settings target. The earlier first candidate's 52% Firefox cold gain is not selected evidence.

All 16 output pairs are byte-exact. The scalar Wasm digest remains `fa6b6381b6d32f2169cddf685e842da813662d6d7f28c6bbc7c6cb57a4b156d0`; only the scalar helper JavaScript changes in the measured package. Its digest is `809e902574ef32c2af32632bffb9290a565d9d0bad64a9a51fb5c1b053468ca5`. Raw reports, provenance, images and the `nearestrowsv2-batch-audit-20260923.json` audit remain outside Git under `/home/mia/mia-cx/ditherette-performance-20260920`.
