...
fn experiment(notes: String) -> io::Result<Experiment> {
...
fn main() -> io::Result<()> {
    ...
    let [output, notes] = args.as_slice() else {
        return Err(io::Error::other(
            "usage: diffusion_cache_plan NEW_JSON HOST_LOAD_NOTES",
        ));
    };
    let bytes = serde_json::to_vec_pretty(&experiment(notes.clone())?).map_err(io::Error::other)?;
    OpenOptions::new()
        .create_new(true)
        .write(true)
        .open(output)?
