...
#[test]
fn warm_calls_share_palettes_and_plans_without_retaining_source_results() {
    ...
    assert_eq!(processor.preparation.stats().1, 2);
    assert_eq!(
    ...
    assert_eq!(processor.preparation.stats().1, 4);
...
fn final_copy_failures_drop_pending_and_active_scratch_but_keep_previous_hits() {
    ...
    assert_eq!(processor.preparation.stats(), (0, 0, 2, 0, 0));
    io.fail = false;
    ...
    assert_eq!(processor.preparation.stats(), (2, 1, 5, retained, 0));
    io.fail = false;
    assert_eq!(
        processor.process(process(&reversed), &mut io).unwrap(),
        [0; 3]
    );
    assert_eq!(processor.preparation.stats().0, 3);
    assert_eq!(processor.preparation.stats().2, 6);
}
