...
pub(crate) const LANCZOS3_LOBES: f64 = 3.0;

/// Resizes RGBA with a fixed-radius Lanczos3 filter.
...
/// Resizes into a caller-provided output buffer with a fixed-radius Lanczos3 filter.
...
// TODO(perf): Precompute exactly six x indices/weights per output column and six
// y indices/weights per output row for normal upscales; variable tap Vecs are
// unnecessary when the support radius is fixed.
...
/// Straightforward reference implementation for Lanczos3 resize.
#[doc(hidden)]
pub fn resize_rgba_lanczos3_reference(
    source_rgba: &[u8],
    source_dimensions: ImageDimensions,
    output_dimensions: ImageDimensions,
) -> Result<Vec<u8>, ProcessingError> {
...
/// Allocation-free form of the Lanczos3 reference implementation.
#[doc(hidden)]
pub fn resize_rgba_lanczos3_reference_into(
    source_rgba: &[u8],
    source_dimensions: ImageDimensions,
    output_dimensions: ImageDimensions,
    output_rgba: &mut [u8],
...
#[cfg(test)]
mod tests {
    use super::resize_rgba_lanczos3;
