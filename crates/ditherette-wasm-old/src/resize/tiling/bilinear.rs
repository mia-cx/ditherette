...
// TODO(perf): Densely sweep tiny bilinear outputs around the current
// scalar/2-worker crossover and tighten these output-pixel cutoffs. Keep the
// production formula dimension-derived; scale labels from sweep reports are
// only breadcrumbs for choosing which dimensions to measure next.
...
fn resize_rgba_bilinear_with_plan_into(
    ...
    process_row_bands_with_plan(output_rgba, tiling_plan, |band, output_rows| {
        // TODO(perf): Benchmark per-worker/thread-local vertical scratch reuse
        // for tiled bilinear. Row-band tiling is now hot enough that allocating
        // one f32 row per band may show up in repeated-resize or small-output
        // workloads even though the single-resize Criterion wins are large.
