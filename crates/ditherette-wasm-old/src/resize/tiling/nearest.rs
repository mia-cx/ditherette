...
pub(crate) fn dynamic_nearest_tiling_plan(
    ...
    let tiling = if output_pixels < NEAREST_TINY_OUTPUT_PIXEL_LIMIT {
        return Ok(None);
    } else if is_strong_exact_integer_downscale(source_dimensions, output_dimensions) {
        return Ok(None);
...
fn is_strong_exact_integer_downscale(
    source_dimensions: ImageDimensions,
    output_dimensions: ImageDimensions,
) -> bool {
    is_exact_integer_downscale(source_dimensions, output_dimensions)
        && (source_dimensions.width() / output_dimensions.width() >= 4
            || source_dimensions.height() / output_dimensions.height() >= 4)
}
...
mod dynamic_plan_tests {
    ...
    #[test]
    fn dynamic_plan_keeps_strong_exact_downscales_scalar() {
        assert!(
            dynamic_nearest_tiling_plan(dimensions(2600, 4168), dimensions(650, 1042))
                .unwrap()
                .is_none()
        );
        assert!(
            dynamic_nearest_tiling_plan(dimensions(2600, 4168), dimensions(325, 521))
                .unwrap()
                .is_none()
        );
    }
