...
pub(crate) fn dynamic_area_tiling_plan(
    ...
    if output_pixels < AREA_TINY_OUTPUT_PIXEL_LIMIT
        ...
        || is_exact_integer_upscale(source_dimensions, output_dimensions)
    {
        return Ok(None);
    }
    ...
    // dimensions matter more than nominal scale labels: identity and tiny
    // outputs stay scalar, exact strong minifiers stay scalar unless they are
    // large enough to repay row-band work, large near-source resizes prefer the
    // default five-worker shape, and fractional downscales prefer four bands.
...
mod tests {
    ...
    #[test]
    fn dynamic_plan_keeps_identity_scalar_and_uses_default_shape_for_near_source() {
        assert!(
            dynamic_area_tiling_plan(dimensions(2600, 4168), dimensions(2600, 4168))
                .unwrap()
                .is_none()
        );
        ...
        let near_source_plan =
            dynamic_area_tiling_plan(dimensions(2600, 4168), dimensions(2470, 3959))
                .unwrap()
                .unwrap();
        assert_eq!(near_source_plan.min_rows_per_band, 192);
        assert_eq!(near_source_plan.max_workers, 8);
    }
    ...
    #[test]
    fn dynamic_plan_tiles_large_fractional_minify_and_keeps_tiny_outputs_scalar() {
        let plan = dynamic_area_tiling_plan(dimensions(2600, 4168), dimensions(520, 833))
            .unwrap()
            .unwrap();
        assert_eq!(plan.min_rows_per_band, 64);
        assert_eq!(plan.max_workers, 4);
