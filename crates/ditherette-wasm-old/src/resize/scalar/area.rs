...
/// Scalar area implementation.
#[allow(dead_code)]
pub fn resize_rgba_area_scalar(
    source_rgba: &[u8],
...
pub fn resize_rgba_area_scalar_into(
    ...
    // NOTE(perf): area dispatches fractional minification to an old-area-style
    ...
    for output_y in 0..output_height {
        ...
        for output_x in 0..output_width {
            ...
            for &(source_y, y_weight) in y_weights {
                ...
                // and 0.125x in `pnpm bench:resize:area`; keep the direct
                ...
                for &(source_x, x_weight) in x_weights {
                    ...
                    // locals preserved correctness but regressed most area
            ...
            // once changed f64 rounding and failed area correctness at 0.95x;
...
fn resize_exact_integer_upscale_into(
    ...
    for source_y in 0..source_height {
        ...
        let (first_output_row, repeated_output_rows) = output_rgba
        ...
        for (source_pixel, output_pixels) in source_row
            ...
            .zip(first_output_row.chunks_exact_mut(rgba::RGBA_CHANNEL_COUNT * x_step))
        {
...
// resize/shared/area.rs preserved correctness but regressed area exact
// downscales by ~3-11% in `pnpm bench:resize:area --scale-group
// exact-downscale --baseline area_accepted`; keep local kernels so LLVM can
...
impl SourceRange {
    ...
    fn overlap_with(self, source_coordinate: usize) -> f64 {
        ...
        // 0.95x, 0.5x, 0.375x, 0.25x, and 0.125x in area.
...
fn round_channel(value: f64) -> u8 {
    ...
    // rounding and failed area correctness; keep per-channel division.
