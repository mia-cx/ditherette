> Legacy Rust/Wasm prototype preserved for reference while the new `ditherette-wasm` crate is specified from a clean slate.
...
```sh
pnpm crit:resize:filters
```

Run one filter at a time:

```sh
pnpm crit:resize:nearest
pnpm crit:resize:nearest:aa
pnpm crit:resize:bilinear
pnpm crit:resize:trilinear
pnpm crit:resize:bicubic
pnpm crit:resize:lanczos2
pnpm crit:resize:lanczos2-scale-aware
pnpm crit:resize:lanczos3
pnpm crit:resize:lanczos3-scale-aware
pnpm crit:resize:area
pnpm crit:resize:box
pnpm crit:resize:antialias
```
...
pnpm crit:resize:bilinear --measurement-time 30 --sample-size 100 --warm-up-time 5
pnpm crit:resize:filters --measurement-time 30 --sample-size 100 --warm-up-time 5
```
...
pnpm crit:cmp --compare resize:bilinear:scalar --to resize:bicubic:scalar
```

Compare in-process resize libraries without Criterion or CLI tool startup overhead:

```sh
pnpm bench:resize:shootout --filter nearest,bilinear,lanczos3,area --scales 2,1,0.99,0.95,0.8,0.5,0.25
```

The shootout decodes the fixture once, reuses RGBA buffers, and compares Ditherette with Rust libraries (`image`, `fast_image_resize`, and `resize` where that filter exists). External CLIs are intentionally excluded so process startup and file I/O do not muddy kernel timings.

Measure row-band overhead directly and in area/nearest context on a small scale subset (`2x`, `1x`, `0.8x`, `0.5x`, `0.25x`, `0.125x`):

```sh
pnpm crit:resize:tiling-overhead
```
