...
const benchmarkConfigs = {
	nearest: {
		bench: 'crit_resize_nearest',
		groupPrefix: 'resize_nearest/celeste_rgba/',
		variants: ['baseline'],
		referenceMode: 'none'
	},
	bilinear: {
		bench: 'crit_resize_bilinear',
...
const scaleOrder = new Map(
	['2x', '0.95x', '0.875x', '0.8x', '0.75x', '0.625x', '0.5x', '0.375x', '0.25x', '0.125x'].map((scale, index) => [scale, index])
);
...
const cargo = spawnSync(
	...
	[
		...
		'--',
		'--noplot',
		...criterionArgsWithDefaults(args.criterionArgs)
	],
	{
		cwd: root,
		stdio: 'inherit',
		env: {
			...process.env,
			...(resizeFilterEnv() ? { RESIZE_FILTER: resizeFilterEnv() } : {}),
...
function compareSummaries(left, right) {
	const leftParts = caseParts(left.caseId);
	const rightParts = caseParts(right.caseId);
	return (
		leftParts.fixture.localeCompare(rightParts.fixture, undefined, { numeric: true }) ||
		scaleIndex(leftParts.scale) - scaleIndex(rightParts.scale) ||
		leftParts.output.localeCompare(rightParts.output, undefined, { numeric: true }) ||
...
function caseParts(caseId) {
	...
	const fixture = parts.length > 1 ? parts.slice(0, -1).join('/') : '';
	const leaf = parts.at(-1) ?? '';
	const [scale = '', ...outputParts] = leaf.split('-');
	return { fixture, scale, output: outputParts.join('-') };
}

function scaleIndex(scale) {
	return scaleOrder.get(scale) ?? Number.MAX_SAFE_INTEGER;
}
...
function parseArgs(argv) {
	...
	return { kernel, filter, scaleGroup, baseline, allFixtures, cargoArgs, criterionArgs };
}
