...
const implementations = new Set(['reference', 'scalar', 'tiling']);
...
function runBenchmark({ label, triple, criterionMode, criterionArgs, scaleGroup, compareId }) {
	...
	cargoArgs.push('--', '--noplot', ...criterionMode, ...criterionArgs);
...
function validateComparison(left, right, criterionArgs) {
	...
	for (const criterionArg of criterionArgs) {
		if (criterionArg === '--baseline' || criterionArg.startsWith('--baseline=')) {
			throw new Error('Do not pass --baseline to bench:cmp; it is generated from --compare/--to.');
		}
		if (criterionArg === '--save-baseline' || criterionArg.startsWith('--save-baseline=')) {
			throw new Error('Do not pass --save-baseline to bench:cmp; it is generated from --compare/--to.');
		}
	}
}
...
function helpText() {
	return `Usage: pnpm bench:cmp --compare resize:area:reference --to resize:area:scalar [-- criterion options]
...
Supported resize implementations: reference, scalar, tiling.
Examples:
