...
export function run(program, args, cwd, options = {}) {
	return execFileSync(program, args, { cwd, env: cargoEnv(), encoding: 'utf8', maxBuffer: 32 * 1024 * 1024, ...options });
}

export function verifyCompiler() {
	const version = run('rustc', [`+${TOOLCHAIN}`, '-vV'], HERE);
	for (const line of ['release: 1.97.0', 'commit-hash: 2d8144b7880597b6e6d3dfd63a9a9efae3f533d3', 'LLVM version: 22.1.6']) {
		if (!version.split('\n').includes(line)) throw new Error(`Reference compiler mismatch: expected ${line}`);
	}
	return version.trim();
}
...
export function verifyBuildConfiguration(root) {
	...
	for (const path of [join(cargoHome, 'config'), join(cargoHome, 'config.toml'), join(tmpdir(), '.cargo/config'), join(tmpdir(), '.cargo/config.toml'), '/.cargo/config', '/.cargo/config.toml']) {
	...
	for (const directory of ['', 'crates', CRATE, 'crates/ditherette-bench']) {
		for (const name of ['.cargo/config', '.cargo/config.toml']) {
			if (existsSync(join(root, directory, name))) throw new Error(`Unaudited Cargo configuration: ${directory}/${name}`);
		}
	}
	if (existsSync(join(root, CRATE, 'build.rs'))) throw new Error('Reference crate cannot acquire a build script');
	for (const path of ['Cargo.toml', 'crates/Cargo.toml']) {
		if (existsSync(join(root, path))) throw new Error(`Unaudited enclosing Cargo workspace: ${path}`);
	}
}
...
export function dependencyContext(root, manifest, target) {
	const metadata = JSON.parse(run('cargo', [`+${TOOLCHAIN}`, 'metadata', '--manifest-path', join(root, manifest), '--format-version', '1', '--locked', '--all-features', '--filter-platform', target], tmpdir()));
	...
	if (!core || core.edition !== '2021' || core.targets.some((t) => t.kind.includes('custom-build'))) throw new Error('Changed reference crate edition or build script');
	const lib = core.targets.find((t) => t.kind.includes('rlib'));
	if (!lib || lib.src_path !== join(root, CRATE, 'src/lib.rs')) throw new Error('Changed reference crate root');
	const pending = REFERENCE_DEPS.map((name) => {
		const dependency = nodes.get(core.id).deps.find((d) => d.name === name);
		if (!dependency || packages.get(dependency.pkg).name !== name) throw new Error(`Reference dependency redirected: ${name}`);
	...
	const checksums = new Map(lock.split('[[package]]').slice(1).map((part) => {
	...
	return [...closure].sort().map((id) => {
		...
		if (source !== 'registry+https://github.com/rust-lang/crates.io-index' || !checksum) throw new Error(`Unpinned reference dependency: ${id}`);
		return { name, version, source, checksum, features: [...nodes.get(id).features].sort(), dependencies: [...nodes.get(id).dependencies].sort() };
	});
}
...
export function dependencySnapshot(root) {
	...
	for (const manifest of [`${CRATE}/Cargo.toml`, 'crates/ditherette-bench/Cargo.toml']) {
		for (const target of TARGETS) result[`${manifest}:${target}`] = dependencyContext(root, manifest, target);
	}
	return result;
}
...
export function verifyDependencies(root, expected) {
	...
	for (const [context, entries] of Object.entries(actual)) {
		if (JSON.stringify(entries) !== JSON.stringify(expected[context])) {
			const changed = entries.filter((entry) => JSON.stringify(entry) !== JSON.stringify(expected[context]?.find((e) => e.name === entry.name)));
			throw new Error(`Reference dependency/features changed in ${context}:\n${JSON.stringify(changed, null, 2)}`);
		}
	}
}

export function syntaxBinary() {
	run('cargo', [`+${TOOLCHAIN}`, 'build', '--locked', '--manifest-path', join(HERE, 'syntax/Cargo.toml')], tmpdir(), { stdio: ['ignore', 'inherit', 'inherit'] });
	return join(HERE, 'syntax/target/debug/ditherette-freeze-syntax');
}

export function verifySyntax(root, binary = syntaxBinary()) {
	const args = ['root', join(root, CRATE, 'src/lib.rs'), 'core-manifest', join(root, CRATE, 'Cargo.toml'), 'bench-manifest', join(root, 'crates/ditherette-bench/Cargo.toml')];
	for (const role of ['spec', 'prod', 'image']) {
		for (const file of files(root, `${CRATE}/src/${role}`).filter((path) => path.endsWith('.rs'))) args.push(role, join(root, file));
	}
...
export function isolatedCheck(root, role, target, threads = false) {
	...
	try {
		...
		for (const folder of [role, 'image']) cpSync(join(root, CRATE, 'src', folder), join(temporary, 'src', folder), { recursive: true });
		...
		run('cargo', [`+${TOOLCHAIN}`, 'check', '--offline', '--manifest-path', join(temporary, 'Cargo.toml'), '--target', target, ...(threads ? ['--features', 'threads'] : [])], tmpdir(), {
			env: { ...cargoEnv(), CARGO_TARGET_DIR: join(HERE, 'syntax/target/isolation') }, stdio: ['ignore', 'inherit', 'inherit']
		});
	} finally {
